...
impl DebugDisplay for CSLuaEventManImp {
    fn render_debug(&self, ui: &Ui) {
        ui.header("Lua Event Observer", || {
            self.lua_event_observer.render_debug(ui);
        });

        ui.header("Lua Event Proxy", || {
            self.lua_event_proxy.render_debug(ui);
        });
        if let Some(event_script_imitation) = self.lua_event_script_imitation.as_ref() {
            ui.header("Lua Event Script Imitation", || {
                event_script_imitation.render_debug(ui);
            });
        } else {
            ui.text("No Lua Event Script Imitation");
        }
    }
}
...
impl DebugDisplay for CSLuaEventProxy {
    fn render_debug(&self, ui: &Ui) {
        ui.text(format!("Is net message: {}", self.is_net_message));
        ui.text(format!(
            "Disable event networking: {}",
            self.disable_event_networking
        ));
        ui.text(format!("Is repeat message: {}", self.is_repeat_message));
        ui.text(format!("Is load-wait: {}", self.is_load_wait));
        ui.text(format!(
            "Is lobby-state-client: {}",
            self.is_lobby_state_client
        ));

        ui.text(format!("Summon param type: {:?}", self.summon_param_type));

        ui.header("Control flags", || {
            self.control_flags.render_debug(ui);
        });

        ui.header("Event message map", || {
            self.lua_event_msg_map.render_debug(ui);
        });
    }
}
...
impl DebugDisplay for CSLuaEventCondition {
    fn render_debug(&self, ui: &Ui) {
        ui.text(format!("Condition id: {}", self.condition_id));
        if let Ok(lua_event_id) = LuaEventId::try_from(self.condition_id as u32) {
            ui.text(format!("  (LuaEventId: {:?})", lua_event_id));
        }
        ui.text(format!("Event group: {}", self.event_group));
        ui.text(format!("Stop execution: {}", self.stop_execution));
        ui.text(format!("Is deleted: {}", self.is_deleted));
        ui.text(format!("Execute repetition: {:?}", self.execute_repetition));
    }
}
...
impl DebugDisplay for EventMsgExecListEntry {
    fn render_debug(&self, ui: &Ui) {
        ui.text(format!("Event group: {}", self.event_group));
        ui.text(format!("Arg1: {}", self.arg1));
        if let Ok(lua_event_id) = LuaEventId::try_from(self.arg1) {
            ui.text(format!("  (LuaEventId: {:?})", lua_event_id));
        }
        ui.text(format!("Arg2: {}", self.arg2));
        ui.text(format!("Arg3: {}", self.arg3));
        ui.text(format!(
            "Repetition: {:?}",
            self.repetition as LuaScriptExecuteRepetition
        ));
        ui.text(format!("Is net message: {}", self.is_net_message));
        ui.text(format!("Is repeat message: {}", self.is_repeat_message));
        ui.text(format!("Is deleted: {}", self.is_deleted));
...
impl DebugDisplay for CSLuaEventMsgExec_String {
    fn render_debug(&self, ui: &Ui) {
        ...
        ui.text(format!("Event message: {}", self.event_msg));
    }
}

impl DebugDisplay for CSLuaEventScriptImitation {
    fn render_debug(&self, ui: &Ui) {
        ui.text(format!("Clear boss id: {}", self.clear_boss_id));
        ui.text(format!("Is kill host: {}", self.is_kill_host));
        ui.text(format!("Death state: {:?}", self.death_state));
        ui.text(format!(
            "Is death-penalty-skip: {}",
            self.is_death_penalty_skip
        ));
        ui.text(format!("Should reset world: {}", self.should_reset_world));
        ui.text(format!(
            "Should reset character: {}",
            self.should_reset_character
        ));
        ui.text(format!("World reset delay: {}", self.world_reset_delay));
        ...
        ui.header("CSDeathRestartEvent", || {
            self.death_restart_event.render_debug(ui);
        });
    }
}
...
impl DebugDisplay for CSDeathRestartEvent {
    fn render_debug(&self, ui: &Ui) {
        ui.text(format!(
            "Full screen message id: {:?}",
            self.full_screen_message_id
        ));
...
impl DebugDisplay for LuaScriptExecuteRepetition {
    fn render_debug(&self, ui: &Ui) {
        ui.text(format!("{:?}", self));
    }
}
