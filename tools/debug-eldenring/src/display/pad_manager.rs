                ...
                ui.header(format!("CSPadEntryMap [{}]", index), || {
                    ...
                    for (index, pair) in tree.iter().enumerate() {
                        ui.header(format!("Pair [{}]", index), || {
                            ui.text(format!("Key: {:#X}", pair.first));
                            ui.header("CSPad", || {
                                let cs_pad = unsafe { pair.second.entry.as_ref() };
                    ...
                    for (index, pair) in tree.iter().enumerate() {
                        ui.header(format!("Pair [{}]", index), || {
                            ...
                            ui.header("CSKeyAssign", || {
                                let key_assign = unsafe { pair.second.as_ref() };
        ...
        ui.table(
            ...
            |ui: &hudhook::imgui::Ui, _, index: usize| {
                ...
                for offset in 0..4 {
                    ui.table_next_column();
                    let idx = index + offset;
                    if let Some(value) = self.vector.get(idx) {
                        ui.text(format!("{:.4}", value));
                    }
                }
            },
        );
        ...
        ui.header("DIRECTINPUT_KEYBOARD", || {
            ...
            ui.table(
                ...
                |ui: &hudhook::imgui::Ui, _, index: usize| {
                    ...
                    for offset in 0..4 {
                        ui.table_next_column();
                        let first = index + offset;
                        if first < len {
                            let state = self.is_key_pressed(first);
        ...
        ui.header("InputCodeCheck", || {
            ...
            ui.table(
                ...
                |ui, _, pair| {
                    ...
                    ui.text(format!("{}", pair.first));
                    ui.table_next_column();
                    ui.text(format!("{}", pair.second.state_1));
                    ui.table_next_column();
                    ui.text(format!("{}", pair.second.state_2));
                },
            );
        });
...
impl DebugDisplay for CSKeyAssign {
    fn render_debug(&self, ui: &hudhook::imgui::Ui) {
        ui.header("virtual_input_data_index_map", || {
            ...
            ui.table(
                "virtual_input_data_index_map_TABLE",
                [
                    TableColumnSetup::new("Mapped Input"),
        ...
        ui.header("mouse_button_states_map", || {
            let tree = &self.mouse_button_states_map;
            ui.table(
                "mouse_button_states_map_TABLE",
                [
                    TableColumnSetup::new("Mapped Input"),
                    TableColumnSetup::new("MouseButtonState Index"),
                ],
                tree.iter(),
                |ui, _, pair| {
                    ui.table_next_column();
