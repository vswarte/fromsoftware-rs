...
use crate::{DLMap, UnkDLTree, cs::BlockId, position::HavokPosition};
    ...
    pub fn world_block_info_by_map(&self, block_id: &BlockId) -> Option<&WorldBlockInfo> {
        ...
        match block_id.is_overworld() {
            true => self
                ...
                .find(|b| b.second.block_id == block_id)
                .map(|b| b.second.as_ref()),
            false => self
                .world_block_info()
                .iter()
...
pub struct WorldGridAreaInfo {
    ...
    unkb8: u64,
    unkc0: u64,
    pub small_bases: DLMap<BlockId, WorldBlockInfo>,
}
