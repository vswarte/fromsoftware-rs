...
use crate::cs::{CSModelIns, FieldInsBaseVmt};
...
pub struct CSWorldGeomManBlockData {
    ...
    _pad335: [u8; 3],
    pub geom_event_entity_id_map: DLMap<u32, FieldInsHandle>,
    unk350: usize,
    unk358: usize,
    unk360: usize,
    ladder_geometry: DLVector<()>,
...
impl GeometrySpawnRequest {
    pub fn asset(&self) -> String {
        self.asset_string.to_string().unwrap_or_default()
    }
    ...
    /// # Panics
    ///
    /// Panics if `asset` (in UTF-16 code units, plus the null terminator)
    /// does not fit in the fixed 32-unit buffer.
    pub fn set_asset(&mut self, asset: &str) {
        self.asset_string
            .assign_str(asset)
            .expect("asset name failed to encode as UTF-16")
    }
}
